...
"""
import sys, os, re, py_compile, traceback
sys.path.insert(0, '.')
PASS, FAIL = [], []
def check(case, expect, got, detail=""):
    ok = expect == got; (PASS if ok else FAIL).append(case)
    print(f'  [{"PASS" if ok else "FAIL"}] {case}: expected {expect!r}, got {got!r}' + (f'  {detail}' if detail and not ok else ''))

print("=" * 78); print("F. REGRESSION"); print("=" * 78)

print("\nF1. Every module and every page compiles")
mods = sorted(f for f in os.listdir('.') if f.endswith('.py') and f not in
              ('mirror.py','fixtures.py','access.py','regression.py'))
views = sorted(os.listdir('views')) if os.path.isdir('views') else []
bad = []
for f in mods:
...
for f in views:
    ...
    try: py_compile.compile(os.path.join('views', f), doraise=True)
    except Exception as e: bad.append((f, str(e)[:80]))
...
check(f"all {len(files)} page files referenced by the navigation exist", [],
      sorted(f for f in files if not os.path.exists(f)))
...
check("all six existing Industrial Intelligence pages still exist", [],
      sorted(k for k, f in II.items() if not os.path.exists(f)))
...
for k, f in II.items():
    src = open(f).read()
    if "certipur" in src.lower():
        FAIL.append(f"{k} references certipur")
check("none of the six references CertiPUR", 0,
      sum(1 for f in II.values() if "certipur" in open(f).read().lower()))
