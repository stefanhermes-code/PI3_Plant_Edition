...
if ai_assistant.is_enabled_for_plant(session, run.plant_id):
    st.divider()
    if st.button("Ask PI3 to reason about this", key=f"ask_pi3_root_cause_{obs.id}"):
        change_summary = (
    ...
    if ai_answer:
        st.subheader("🤖 PI3/AI hypothesis")
        st.caption(
            "Generated by the PI3/AI Assistant from the comparison above plus expert notes and "
            "historical cases. A starting point for investigation, not a diagnosis — confirm any "
