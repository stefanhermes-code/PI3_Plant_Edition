...
if ai_assistant.is_enabled_for_plant(session, plant_id):
    ...
    if st.button(
        ...
        prompt = (
            ...
            "cases in the connected knowledge base, to propose a CANDIDATE formulation for "
            "qualified technical evaluation against the target properties requested.\n\n"
            "Phrase this as a candidate recommendation for the reviewer to evaluate and confirm "
            "through their own trial process, addressed directly to the target properties "
            "requested. Where you rely on a specific cost, diff, or correlation figure below, "
            "refer to it explicitly rather than restating the raw ingredient list.\n\n"
            # CR of 19 Aug 2026, section 5.1. The checklist is asked for as
            # "relevant to THIS recommendation" on purpose: a fixed list
            # printed under every answer is read once and skipped afterwards,
            # which is the opposite of what a verification step is for.
            "End with a short section headed 'Verification before trial'. List only the checks "
            "that actually matter for the recommendation you have just made - for example exact "
            "raw material grades, the formulation calculation basis, isocyanate index, raw "
            "material compatibility, catalyst balance, the processing window, the actual "
            "equipment configuration, operating conditions, applicable supplier technical "
            "information, applicable SDS requirements, or site process-safety requirements. "
            "Leave out anything the recommendation does not touch, and say plainly where missing "
