"""Quality test results - the same-source sample rule.

Every result names a sample, and that sample belongs to the same source
record as the result. Production runs, customer trials and optimization
trials alike, at the application layer and at the database layer.

Moved under the Permanent Automated Regression Test Suite CR on 22 August 2026.
Original: tests/test_sample_integrity.py
"""
...
RECORDING = replay("sample_integrity", expected_checks=23)
