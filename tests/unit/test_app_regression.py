"""Application regression - compilation, navigation, analytics, policy, reports.

Every module and page compiles; navigation and the access-control
catalogue agree; the shared analytics functions still import and run; the
pass/fail policy keeps the widths and directions ruled on 21 August; and
the reports module still exposes every report it did before.

Moved under the Permanent Automated Regression Test Suite CR on 22 August 2026.
Original: regression.py - a working script, never in version control until now
"""
...
RECORDING = replay("app_regression", expected_checks=48)
